...
function Header({userInfo, setUserInfo}) {
    ...
    const [stateUserTool, setStateUserTool] = useState(false)
    const toggleUserTool = () => setStateUserTool(prev => !prev);
            ...
            <div className="menu">
                ...
                {userInfo.isAuthenticated === true &&
                    <div className="user-container" style={{ cursor: "pointer" }}>
                        <div className='user-info link' onClick={toggleUserTool}>
                            <img src={`/img/icons/user.svg`} className='icon' />
                            {userInfo.username}
                        </div>
                        <div className={`user-tool ${stateUserTool ? 'block' : 'hidden'}`}>
                            <div>
                                <Link to={`/change-pass`}>
